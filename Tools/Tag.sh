...
default_branch() {
  if git symbolic-ref --quiet --short refs/remotes/origin/HEAD >/dev/null; then
...
require_merged_default_branch() {
  local release_branch current_branch local_sha remote_sha

  release_branch="$(default_branch)"
  current_branch="$(git branch --show-current)"
  if [[ "$current_branch" != "$release_branch" ]]; then
    echo "$repo_name: tagger must be run from $release_branch after the PR is merged; current branch is $current_branch" >&2
    exit 1
  fi
  ...
  if [[ "$local_sha" != "$remote_sha" ]]; then
    echo "$repo_name: local $release_branch is not up to date with origin/$release_branch" >&2
