...
class TestSystem(unittest.TestCase):
    ...
    def test_set_record_instruments(self):
        """
        Test if the time limit for recording instrument activity
        is set correctly.
        """

        # Case 1: Record forever
        self.system.set_record_instruments(-1)
        assert self.system.t_instruments_max_ms == -1
        assert self.system.t_instruments_start_ms == self.system.t_ms

        # Case 2: Record for a finite time
        self.system.set_record_instruments(100)
        assert self.system.t_instruments_max_ms == 100
        assert self.system.t_instruments_start_ms == self.system.t_ms

        # Case 3: Record for zero time
        self.system.set_record_instruments(0)
        assert self.system.t_instruments_max_ms == 0
        assert self.system.t_instruments_start_ms == 0

    def test_instruments_are_recording(self):
        """Test if the correct state of the simulation in terms of the
        recording activity of recording instruments of the System is
        returned."""
        # Case 1: Instruments are not set to record anything
        self.system.set_record_instruments(0)
        assert not self.system.instruments_are_recording()

        # Case 2: Instruments are set to record for a finite time
        self.system.set_record_instruments(100)
        assert self.system.instruments_are_recording()

        # Case 3: Instruments are set to record everything
        self.system.set_record_instruments(-1)
        assert self.system.instruments_are_recording()
    ...
    def test_get_instrument_recordings(self):
        """Test if all instrument recordings are retrieved correctly."""

        # Nothing has been recorded immediately after setup
        data = self.system.get_instrument_recordings()
        assert "t_ms" in data
        assert len(data["t_ms"]) == 0
        for electrode_id in self.system.recording_electrodes:
            assert electrode_id in data
            electrode_data = data[electrode_id]
            electrode = self.system.recording_electrodes[electrode_id]
            # for cell_id in circuit.cells:
            #     assert cell_id in circuit_data
            #     assert len(circuit_data[cell_id]["Vm"]) == 0
        # TODO: Add calcium imaging testing
        # TODO: What happens after a simulation has been run?
