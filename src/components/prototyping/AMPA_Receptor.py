...
class AMPA_Receptor:
    ...
    def __init__(
        ...
        self.tau_r_ms = (
            tau_r_ms_init  # Receptor conductance rise time constant (typical: 0.2 ms)
        )
        ...
        self.a_norm = (
            a_norm_init  # normalizing scale factor so that peak Gsyn_t_pS == g_peak_pS
        )
    ...
    def Gsyn_t_pS(self, t_ms: float) -> float:
        ...
        if t_ms < 0:
            ...
            Gsyn_pS_t = (
                self.g_peak_pS
                * (-exp(-t_ms / self.tau_r_ms) + exp(-t_ms / self.tau_d_ms))
                / self.a_norm
            )
        self.Gsyn_pS = Gsyn_pS_t
        return Gsyn_pS_t
    ...
    def np_Gsyn_t_pS_dbl(self, t_ms: np.ndarray) -> np.ndarray:
        ...
        Gsyn_pS_t = (
            self.g_peak_pS
            * (-np.exp(-t_ms / self.tau_r_ms) + np.exp(-t_ms / self.tau_d_ms))
            / self.a_norm
        )
        self.Gsyn_pS = Gsyn_pS_t
        return Gsyn_pS_t
    ...
    def np_Gsyn_t_pS_mxh(self, t_ms: np.ndarray) -> np.ndarray:
        ...
        Gsyn_pS_t = (
            self.g_peak_pS
            * np.power((1.0 - np.exp(-t_ms / self.tau_r_ms)), self.x)
            * (
                self.d1 * np.exp(-t_ms / self.tau_d_ms)
                + self.d2 * np.exp(-t_ms / self.tau_d2_ms)
                + self.d3 * np.exp(-t_ms / self.tau_d3_ms)
            )
            / self.a_norm
        )
        self.Gsyn_pS = Gsyn_pS_t
    ...
    def numerical_find_a_norm(self, plot_it=False, print_it=False) -> float:
        ...
        for i in range(100):
            ...
            if print_it:
                print("g_diff=%s, a_norm=%s" % (str(g_diff), str(self.a_norm)))
            if abs(g_diff) < 0.1:
                if plot_it:
                    self.plot_it(
                        list(range(i + 1)),
                        [self.g_diffs, self.a_norms],
                        ["g_diff", "a_norm"],
                        ncols=2,
                    )
                return self.a_norm
