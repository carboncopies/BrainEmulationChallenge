...
from .BG_API import BGNES_BS_receptor_create
...
class BS_Aligned_NC(NeuralCircuit):
    ...
    def init_cells(self, domain: Geometry):
        for n in range(self.num_cells):
            ...
            soma = BS_Soma(n, domain_bounds, align="left")
            axon = BS_Axon(
                n, domain_bounds, align="right", soma_radius_um=soma.radius_um
            )
            cell = BS_Neuron(
                str(n),
                soma,
    ...
    def Set_Weight(self, from_to: tuple, method: str):
        ...
        if method == "binary":
            ...
            if to_cell >= len(self.cells):
                raise Exception(
                    "BS_Aligned_NC.Set_Weight: Unknown target cell %d." % to_cell
                )
            from_cell = from_to[0]
            if from_cell >= len(self.cells):
                raise Exception(
                    "BS_Aligned_NC.Set_Weight: Unknown source cell %d." % from_cell
                )
    ...
    def Encode(
        self, pattern_set: list, encoding_method: str, synapse_weight_method: str
    ):
    ...
    def attach_direct_stim(self, tstim_ms: list):
        for stim in tstim_ms:
            ...
            if cell_num >= len(self.cells):
                raise Exception(
                    "BS_Aligned_NC.attach_direct_stim: %d exceeds number of cells."
                    % cell_num
                )
    ...
    def show(self, pltinfo=None):
        if pltinfo is None:
            pltinfo = PlotInfo("Neural circuit %s." % str(self.id))
