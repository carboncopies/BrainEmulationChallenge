...
class Metrics_N1:
    ...
    def build_connectivity_matrix(self, system):
        ...
        # Initialize the a num_neurons x num_neurons connectivity matrix with zeros
        num_neurons = len(all_neurons)
        connectivity_matrix = [[0.0] * num_neurons for _ in range(num_neurons)]

        for connection in system.get_connectivity():
            src_neuron, dst_neuron, weight = connection
            connectivity_matrix[dst_neuron][src_neuron] = weight
