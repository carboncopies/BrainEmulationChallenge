...
def BGNES_handle_response(
    response: requests.Response, caller=str, retstrings: list = ["StatusCode"]
) -> list:
    if response.status_code == 200:
        try:
            ...
            if data["StatusCode"] == 0 or data["StatusCode"] == 3:  # This is temporary!
                ...
                raise Exception(
                    "%s: API returned status code %s." % (caller, data["StatusCode"])
                )
        except Exception as e:
            raise Exception("%s: API did not return expected JSON data." % caller)
    else:
        raise Exception(
            "%s: Failed with GET status %s." % (caller, response.status_code)
        )
...
def BGNES_simulation_create(name: str) -> str:
    ...
    return BGNES_handle_response(response, "BGNES_simulation_create", ["SimulationID"])[
        0
    ]
...
def BGNES_simulation_reset() -> str:
    global AUTHKEY
    reqstr = "/NES/Simulation/Reset?AuthKey=%s&SimulationID=%s" % (AUTHKEY, SIMID)
...
def BGNES_simulation_recordall(MaxRecordTime_ms: float) -> str:
    global AUTHKEY
    reqstr = (
        "/NES/Simulation/RecordAll?AuthKey=%s&SimulationID=%s&MaxRecordTime_ms=%s"
        % (AUTHKEY, SIMID, str(MaxRecordTime_ms))
    )
    response = BGAPI_call(reqstr)
...
def BGNES_get_recording() -> dict:
    ...
    response = BGAPI_call(reqstr)
    jsondatastr = BGNES_handle_response(response, "BGNES_get_recording", ["Recording"])[
        0
    ]
    try:
...
def BGNES_sphere_create(radius_nm: float, center_nm: tuple, name=None) -> str:
    ...
    reqstr = (
        "/NES/Geometry/Shape/Sphere/Create?AuthKey=%s&SimulationID=%s&Radius_nm=%s&Center_nm=%s%s"
        % (AUTHKEY, SIMID, str(radius_nm), json.dumps(list(center_nm)), namestr)
    )
    response = BGAPI_call(reqstr)
...
def BGNES_cylinder_create(
    ...
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Geometry/Shape/Cylinder/Create?AuthKey=%s&SimulationID=%s&Point1Radius_nm=%s&Point1Position_nm=%s&Point2Radius_nm=%s&Point2Position_nm=%s%s"
        % (
            AUTHKEY,
            SIMID,
            str(Point1Radius_nm),
...
def BGNES_box_create(
    CenterPosition_nm: tuple, Dimensions_nm: tuple, Rotation_rad: tuple, name=None
) -> str:
    global AUTHKEY
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Geometry/Shape/Box/Create?AuthKey=%s&SimulationID=%s&CenterPosition_nm=%s&Dimensions_nm=%s&Rotation_rad=%s%s"
        % (
            AUTHKEY,
            SIMID,
...
def BGNES_BS_compartment_create(
    ...
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Compartment/BS/Create?AuthKey=%s&SimulationID=%s&ShapeID=%s&MembranePotential_mV=%s&RestingPotential_mV=%s&SpikeThreshold_mV=%s&DecayTime_ms=%s&AfterHyperpolarizationAmplitude_mV=%s%s"
        % (
            AUTHKEY,
            SIMID,
            str(ShapeID),
    ...
    response = BGAPI_call(reqstr)
    return BGNES_handle_response(
        response, "BGNES_BS_compartment_create", ["CompartmentID"]
    )[0]


def BGNES_connection_staple_create(
    SourceCompartmentID: str, DestinationCompartmentID: float, name=None
) -> str:
    global AUTHKEY
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Connection/Staple/Create?AuthKey=%s&SimulationID=%s&SourceCompartmentID=%s&DestinationCompartmentID=%s%s"
        % (
            AUTHKEY,
            SIMID,
    ...
    response = BGAPI_call(reqstr)
    return BGNES_handle_response(
        response, "BGNES_connection_staple_create", ["StapleID"]
    )[0]
...
def BGNES_BS_receptor_create(
    ...
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Connection/Receptor/Create?AuthKey=%s&SimulationID=%s&SourceCompartmentID=%s&DestinationCompartmentID=%s&Conductance_nS=%s&TimeConstant_ms=%s&ReceptorLocation_nm=%s%s"
        % (
            AUTHKEY,
            SIMID,
            str(SourceCompartmentID),
    ...
    return BGNES_handle_response(response, "BGNES_BS_receptor_create", ["ReceptorID"])[
        0
    ]


def BGNES_DAC_create(
    DestinationCompartmentID: str, ClampLocation_nm: tuple, name=None
) -> str:
    global AUTHKEY
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Tool/PatchClampDAC/Create?AuthKey=%s&SimulationID=%s&DestinationCompartmentID=%s&ClampLocation_nm=%s%s"
        % (
            AUTHKEY,
            SIMID,
            str(DestinationCompartmentID),
...
def BGNES_DAC_set_output_list(
    TargetDAC: str, DACVoltages_mV: list, Timestep_ms: float
) -> str:
    global AUTHKEY
    reqstr = (
        "/NES/Tool/PatchClampDAC/SetOutputList?AuthKey=%s&SimulationID=%s&TargetDAC=%s&DACVoltages_mV=%s&Timestep_ms=%s"
        % (AUTHKEY, SIMID, str(TargetDAC), json.dumps(DACVoltages_mV), str(Timestep_ms))
    )
    response = BGAPI_call(reqstr)
...
def BGNES_ADC_create(
    SourceCompartmentID: str, ClampLocation_nm: tuple, name=None
) -> str:
    global AUTHKEY
    namestr = "" if name is None else "&Name=" + str(name)
    reqstr = (
        "/NES/Tool/PatchClampADC/Create?AuthKey=%s&SimulationID=%s&SourceCompartmentID=%s&ClampLocation_nm=%s%s"
        % (
            AUTHKEY,
            SIMID,
            str(SourceCompartmentID),
...
def BGNES_ADC_set_sample_rate(TargetADC: str, Timestep_ms: float) -> str:
    global AUTHKEY
    reqstr = (
        "/NES/Tool/PatchClampADC/SetSampleRate?AuthKey=%s&SimulationID=%s&TargetADC=%s&Timestep_ms=%s"
        % (AUTHKEY, SIMID, str(TargetADC), str(Timestep_ms))
    )
    response = BGAPI_call(reqstr)
    return BGNES_handle_response(response, "BGNES_ADC_set_sample_rate")[0]


def BGNES_ADC_get_recorded_data(TargetADC: str) -> list:
    global AUTHKEY
    reqstr = (
        "/NES/Tool/PatchClampADC/GetRecordedData?AuthKey=%s&SimulationID=%s&TargetADC=%s"
        % (AUTHKEY, SIMID, str(TargetADC))
    )
    response = BGAPI_call(reqstr)
    return BGNES_handle_response(
        response, "BGNES_ADC_get_recorded_data", ["RecordedData_mV", "Timestep_ms"]
    )
...
def BGNES_QuickStart(
    user: str, passwd: str, scriptversion: str, versionmustmatch: bool, verbose=False
) -> bool:
    """
    ...
    if verbose:
        print("BGNES Version: " + str(version))
    if versionmustmatch:
