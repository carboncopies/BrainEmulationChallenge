...
class Box(Geometry):
    ...
    def equal_slice_bounds(self, n_slices: int, slice: int) -> list:
        ...
        bottomright = (
            self.center_um[0] + half_x,
            y + p_width,
            self.center_um[2] + half_z,
        )
...
class Cylinder(Geometry):
    ...
    def R_at_position(self, xi: float) -> float:
        if xi <= 0.0:
            return self.end0_radius_um
        if xi >= 1.0:
            return self.end1_radius_um
        rdiff = self.end1_radius_um - self.end0_radius_um
        return self.end0_radius_um + xi * rdiff

    def show(self, pltinfo=None):
        if pltinfo is None:
            pltinfo = PlotInfo("Cylinder shape")
        # axis and radius
