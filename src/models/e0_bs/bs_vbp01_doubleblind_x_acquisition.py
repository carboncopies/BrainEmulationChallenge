...
def quickstart(user: str, passwd: str):
    if USENES:
        if not BGNES_QuickStart(
            user, passwd, scriptversion, versionmustmatch=False, verbose=False
        ):
...
def init_spontaneous_activity(bs_acq_system: System):
    ...
    spont_spike_interval_pair = (
        spont_spike_interval_ms_mean,
        spont_spike_interval_ms_stdev,
    )
...
def init_calcium_imaging(bs_acq_system: System):
    calcium_specs = {
        ...
        "fluorescing_neurons": [
            0,
            1,
        ],  # All neurons show up in calcium imaging.
