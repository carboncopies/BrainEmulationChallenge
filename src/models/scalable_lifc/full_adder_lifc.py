...
class Scalable_LIFC:
    Args = None
    DBdata = None
    SimulationCfg, MySim = None
    ClientCfg, ClientInstance = None
    ...
    def __init__(self, description, simulation_title):
        ...
        self.ClientCfg, self.ClientInstance = vbp.ClientFromArgs(DBdata, Args)

        self.SimulationCfg, self.MySim = vbp.NewSimulation(DBdata, ClientInstance, simulation_title, Seed=Args.Seed)
        print('Simulation created')

        self.MySim.SetLIFCAbstractedFunctional(_AbstractedFunctional=True) # needs to be called before building LIFC receptors
        self.MySim.SetSTDP(_DoSTDP=Args.STDP)
...
t_soma_fire_ms = [
    ...
    # TEST SUM
    (100.0, neurons['Cin'].ID),
    (200.0, neurons['P_inB'].ID),
    (300.0, neurons['Cin'].ID),
    (300.0, neurons['P_inB'].ID),
    (400.0, neurons['P_inA'].ID),
    (500.0, neurons['P_inA'].ID),
    (500.0, neurons['Cin'].ID),
    (600.0, neurons['P_inA'].ID),
    (600.0, neurons['P_inB'].ID),
    (700.0, neurons['P_inA'].ID),
    (700.0, neurons['P_inB'].ID),
    (700.0, neurons['Cin'].ID),
]
