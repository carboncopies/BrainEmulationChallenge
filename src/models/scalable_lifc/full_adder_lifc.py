...
class Scalable_LIFC:
    ...
    # sets up a simulation model for circuit
    def __init__(self, description, simulation_title):
        ...
        self.ClientCfg, self.ClientInstance = vbp.ClientFromArgs(self.DBdata, self.Args)

        self.SimulationCfg, self.MySim = vbp.NewSimulation(self.DBdata, self.ClientInstance, simulation_title, Seed=self.Args.Seed)
        print('Simulation created')

        self.MySim.SetLIFCAbstractedFunctional(_AbstractedFunctional=True) # needs to be called before building LIFC receptors
        self.MySim.SetSTDP(_DoSTDP=self.Args.STDP)
    ...
    def full_adder(self):
        ...
        if self.Args.Reload:
            ...
            connection_patterns = {
                'Cin_IC0': (self.neurons['Cin'], self.neurons['I_C0'], 'AMPA', 0, 0.5, 6.0, 20, to_I, 5.0, 'None', 0, 0, 0, 0, False),
                'Cin_PC1': (self.neurons['Cin'], self.neurons['P_C1'], 'AMPA', 0, 0.5, 3.0, 20, to_and, 10.0, 'None', 0, 0, 0, 0, False),
                'Cin_Sum': (self.neurons['Cin'], self.neurons['Sum'], 'AMPA', 0, 0.5, 6.0, 20, to_xor, 30.0, 'None', 0, 0, 0, 0, False),
                
                'PinA_IA0': (self.neurons['P_inA'], self.neurons['I_A0'], 'AMPA', 0, 0.5, 3.0, 20, to_I, 0.0, 'None', 0, 0, 0, 0, False),
                'PinA_PB0': (self.neurons['P_inA'], self.neurons['P_B0'], 'AMPA', 0, 0.5, 5.0, 20, to_xor, 6.0, 'None', 0, 0, 0, 0, False),
                'PinA_PC0': (self.neurons['P_inA'], self.neurons['P_C0'], 'AMPA', 0, 0.5, 3.0, 20, to_and, 1.0, 'None', 0, 0, 0, 0, False),
                
                'PinB_IA0': (self.neurons['P_inB'], self.neurons['I_A0'], 'AMPA', 0, 0.5, 3.0, 20, to_I, 0.0, 'None', 0, 0, 0, 0, False),
                'PinB_PB0': (self.neurons['P_inB'], self.neurons['P_B0'], 'AMPA', 0, 0.5, 5.0, 20, to_xor, 6.0, 'None', 0, 0, 0, 0, False),
                'PinB_PC0': (self.neurons['P_inB'], self.neurons['P_C0'], 'AMPA', 0, 0.5, 3.0, 20, to_and, 1.0, 'None', 0, 0, 0, 0, False),
                
                'IA0_PB0': (self.neurons['I_A0'], self.neurons['P_B0'], 'GABA', -70, 0.5, 13, 40, I_to_xor, 0.0, 'None', 0, 0, 0, 0, False),
                
                'PB0_IC0': (self.neurons['P_B0'], self.neurons['I_C0'], 'AMPA', 0, 0.5, 3.0, 20, to_I, 1.0, 'None', 0, 0, 0, 0, False),
                'PB0_PC1': (self.neurons['P_B0'], self.neurons['P_C1'], 'AMPA', 0, 0.5, 3.0, 20, to_and, 0.0, 'None', 0, 0, 0, 0, False),
                'PB0_Sum': (self.neurons['P_B0'], self.neurons['Sum'], 'AMPA', 0, 0.5, 6.0, 20, to_xor, 10.0, 'None', 0, 0, 0, 0, False),
                
                'IC0_Sum': (self.neurons['I_C0'], self.neurons['Sum'], 'GABA', -70, 0.1, 10, 40, I_to_xor, 0.0, 'None', 0, 0, 0, 0, False),
                
                'PC0_Cout': (self.neurons['P_C0'], self.neurons['Cout'], 'AMPA', 0, 0.5, 3.0, 20, to_or, 10.0, 'None', 0, 0, 0, 0, False),
                'PC1_Cout': (self.neurons['P_C1'], self.neurons['Cout'], 'AMPA', 0, 0.5, 3.0, 20, to_or, 1.0, 'None', 0, 0, 0, 0, False),
            }
    ...
    def set_simulation(self):
        self.MySim.ModelSave('LIFtest')

        # Set stimulation times for full adder testing
        T = 800
        ...
        # Full adder test stimulation pattern
        t_soma_fire_ms = [
            # TEST FIRST XOR LOGIC
            # (100.0, neurons['P_inA'].ID), 
            # (200.0, neurons['P_inB'].ID),  
            # (300.0, neurons['P_inA'].ID), 
            # (300.0, neurons['P_inB'].ID),

            # TEST SUM
            (100.0, self.neurons['Cin'].ID),
            (200.0, self.neurons['P_inB'].ID),
            (300.0, self.neurons['Cin'].ID),
            (300.0, self.neurons['P_inB'].ID),
            (400.0, self.neurons['P_inA'].ID),
            (500.0, self.neurons['P_inA'].ID),
            (500.0, self.neurons['Cin'].ID),
            (600.0, self.neurons['P_inA'].ID),
            (600.0, self.neurons['P_inB'].ID),
            (700.0, self.neurons['P_inA'].ID),
            (700.0, self.neurons['P_inB'].ID),
            (700.0, self.neurons['Cin'].ID),
        ]

        timeneuronpairs_list = [(t, neuron_id) for t, neuron_id in t_soma_fire_ms]
        self.MySim.SetSpecificAPTimes(timeneuronpairs_list)
        print('Simulation stimulation specified')

        # Run simulation and record membrane potential
        self.MySim.RecordAll(-1)
        self.MySim.RunAndWait(Runtime_ms=T, timeout_s=100.0)
        print('Functional stimulation completed')

        recording_dict = self.MySim.GetRecording()
        print('Recorded data retrieved')

        self.cells = {
            'Cin': self.neurons['Cin'],
            'P_inA': self.neurons['P_inA'],
            'P_inB': self.neurons['P_inB'],

            'I_A0': self.neurons['I_A0'],

            'P_B0': self.neurons['P_B0'],
            
            'I_C0': self.neurons['I_C0'],
            'P_C0': self.neurons['P_C0'],
            'P_C1': self.neurons['P_C1'],

            'Sum': self.neurons['Sum'],
            'Cout': self.neurons['Cout'],
        }
    ...
    def record(self):
        ...
        def find_cell_with_id(id:int) -> str:
            ...
            for n in self.cells.keys():
                if self.cells[n].ID == id:
        ...
        if isinstance(recording_dict, dict):
            ...
            if "Recording" in recording_dict and recording_dict["Recording"] is not None:
                ...
                if 'neurons' in recording:
                    ...
                    # Generate plot
                    print(f"\nGenerating plot in {self.savefolder}")
                    plot_recorded(
                        self.savefolder,
...
def main():
    test = Scalable_LIFC("LIFCNeuron test script", "Scalable test")
    test.full_adder()
    test.set_simulation()
    test.record()
