...
Parser.add_argument("-jitter_ms",default=0.0,type=float,help="Random timing jitter in ms (0 = in-domain)")
...
def SpikeInputNeuronsAt(InputID: str, t_ms: float):
    for n in XORInOutIdentifiers[InputID]:
        if Args.jitter_ms > 0.0:
            # Add random jitter between -jitter_ms and +jitter_ms
            t_perturbed = t_ms + random.uniform(-Args.jitter_ms, Args.jitter_ms)
            t_perturbed = max(0.0, t_perturbed)  # no negative times
        else:
            # Original in-domain behavior
            t_perturbed = t_ms
            
        t_soma_fire_ms.append((t_perturbed, n))
...
try:
    ...
    if Args.groundtruth:
        ...
        csv_path = f"{savefolder}/sub-Vm{suffix}.csv"
...
try:
    ...
    if Args.groundtruth:
        ...
        spike_csv_path = f"{savefolder}/sub-spikes{suffix}.csv"
