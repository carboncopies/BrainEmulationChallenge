...
# XOR_GT_H5_PATH / XOR_SUB_H5_PATH: optional absolute or cwd-relative HDF5 paths.
# Defaults: output/GT/groundtruth.h5 (GT); SUB defaults to same file as GT if unset or missing.
try:
    _def_gt = os.path.join(current_dir, "output", "GT", "groundtruth.h5")
    h5_file_path = os.environ.get("XOR_GT_H5_PATH", _def_gt)
    if not os.path.isabs(h5_file_path):
        h5_file_path = os.path.join(current_dir, h5_file_path)

    sub_h5_path = os.environ.get("XOR_SUB_H5_PATH", "").strip()
    if sub_h5_path and not os.path.isabs(sub_h5_path):
        sub_h5_path = os.path.join(current_dir, sub_h5_path)
    ...
    if sub_h5_path and os.path.isfile(sub_h5_path):
        sub_data = pd.read_hdf(sub_h5_path, "/data")
        sub_spikes = pd.read_hdf(sub_h5_path, "/spikes_raw")
        print("Submission (SUB) loaded from:", sub_h5_path)
    else:
        sub_data = gt_data.copy()
        sub_spikes = gt_spikes.copy()
        if sub_h5_path:
            print("WARN: XOR_SUB_H5_PATH set but file not found; SUB = GT copy.", sub_h5_path)
    ...
    _sidecar = os.path.join(os.path.dirname(h5_file_path), "network_config.json")
    if os.path.isfile(_sidecar):
        with open(_sidecar, "r") as f:
            net_config = json.load(f)
    else:
        acq_folders = sorted(glob.glob("output/*-acquisition"))
        if not acq_folders:
            raise FileNotFoundError("No acquisition folders found in output/ and no network_config beside GT h5")
