...
usage() {
    echo "Usage: $0 [-R] [-p port] [-x execute-at] [-s subdivide] [-r resolution] [-f modelfile] [-m modelname] [-n] [-S] [-M] [-F baseURL] [-i simID]"
    ...
    echo "  -p  port number (default 8000)"
    ...
    echo ""
    echo "  ./Run.sh -R -p 8001"
    echo "  Same as above, but uses port 8001 instead of default 8000."
    echo ""
    echo "  ./Run.sh -a"
...
while getopts "Rp:s:r:f:m:x:nSMF:i:" opt; do
  case ${opt} in
    R )
      host="-Host $Rhost"
      port="-Port $Rport"
