...
class AIF_DistributedAutoAssociative_NC(NeuralCircuit):
    ...
    def __init__(
        ...
        ToDo(
            "Randal",
            "Review Zebra Finch literature to extract gist of song memory structure.",
        )
        ToDo(
            "Randal",
            "Produce a simplified auto-associative NN structure inspired by ZF song memory.",
        )
        ToDo(
            "Randal",
            "Outline structure, encoding and retrieval methods for AI Finch LTM.",
        )
        ToDo(
            "VBP Team",
            "Implement LTM user class based on specs, using proposed low-level modeling resources.",
        )
        ToDo(
            "NES Team",
            "Implement low-level modeling resources and API as required by AI Finch LTM VBP specs.",
        )
...
class NotesToSound(Region):
    ...
    def __init__(self, id: str, soundlib="notes.wav"):
        super().__init__(id=id)
...
class RegionPath:
    ...
    def __init__(self, src: Region, dst: Region, method):
        pass
...
def init(melodies: list) -> dict:
    ...
    NUM_NODES = 10000
    aif_mem = AIF_DistributedAutoAssociative_NC(
        id="AI Finch LTM NC", num_nodes=NUM_NODES
    )
    aif_mem_region = BrainRegion(
        id="AI Finch LTM", shape=Box(dims_um=(6.0, 20.0, 20.0)), content=aif_mem
    )
    ...
    aif_mem.Encode(
        pattern_set=songs, encoding_method="instant", synapse_weight_method="binary"
    )
    ...
    aif_in = RegionPath(src=aif_cue, dst=aif_mem_region, method=["1-1", "patchclamp"])
    aif_resp = SpikeMonitor(id="Song retrieval spike monitor")
    aif_out = RegionPath(src=aif_mem_region, dst=aif_resp, method=["1-1", "patchclamp"])

    aif_in.show()
    aif_out.show()

    # 4. Specify melody sound production:
    singing_finch = NotesToSound(
        id="AI Finch singing voice", soundlib="finch_trills.wav"
    )
    aif_vocalization = PatternMapper(src=aif_resp, dst=singing_finch, method="nearest")

    # 5. Specify known ground-truth "God's eye" data output:
    godseye_struc = Imager2D(
        [
            aif_mem_region,
        ]
    )
    godseye_state = StateReader(
        [
            aif_mem_region,
        ]
    )
    godseye_dyn = ActivityRecorder(
        [
            aif_mem_region,
        ]
    )
    ...
    aif_system = {
        ...
        "regions": {
            ...
            "cue": aif_cue,
            "resp": aif_resp,
            "singing": singing_finch,
        },
        "pathways": {
            "cue": aif_in,
            "resp": aif_out,
            "singing": aif_vocalization,
        },
        "inspectors": {
            "struc": godseye_struc,
            "state": godseye_state,
